...
run_snapshot_script() {
  ...
  env \
    ...
    RELEASE_NAME="obaas" \
    RELEASE_REVISION="7" \
    TARGET_VERSION="0.142.1" \
    SNAPSHOT_TIMEOUT="20m" \
    MARKER_SECRET_NAME="obaas-signoz-upgrade-0-142-1-stage1" \
...
assert_not_contains "${stage1_upgrade}" 'image: docker.io/signoz/signoz:v0.142.1'
...
assert_contains "${TEST_ROOT}/success/snapshot-1.yaml" 'obaas.oracle.com/upgrade-target: "0.142.1"'
