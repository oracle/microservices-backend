...
helm template obaas "$chart" -n obaas --is-upgrade >"$tmp"
for image in 'signoz/signoz:v0.142.1' 'signoz/signoz-otel-collector:v0.144.10' 'clickhouse/clickhouse-server:25.12.5'; do grep -qF "$image" "$tmp"; done
if helm template obaas "$chart" -n obaas --is-upgrade \
  --set signozUpgrade.mode=destructive-replace >"$tmp" 2>&1; then exit 1; fi
grep -qF 'signozUpgrade.confirmDataLoss=true is required' "$tmp"
...
grep -qF 'Deleting existing system dashboard:' "$tmp"
