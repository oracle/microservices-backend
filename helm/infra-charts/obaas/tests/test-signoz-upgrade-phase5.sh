...
SIGNOZ_ARCHIVE="${CHART_DIR}/charts/signoz-0.142.1.tgz"
...
assert_contains "${CHART_DIR}/Chart.yaml" 'version: "0.142.1"'
assert_contains "${CHART_DIR}/Chart.lock" 'version: 0.142.1'
...
[ "$(shasum -a 256 "${SIGNOZ_ARCHIVE}" | awk '{print $1}')" = \
  "4b0088757612584d7ff5e596ab0a2ffc4d1f90e43edc6bf7e179208fca1b2277" ] || {
  echo "SigNoZ 0.142.1 chart digest does not match the official artifact" >&2
  exit 1
}
assert_contains "${signoz_values}" 'tag: v0.142.1'
assert_contains "${signoz_values}" 'tag: v0.144.10'
...
assert_contains "${IMAGE_LIST}" 'docker.io/signoz/signoz:v0.142.1'
...
run_resource() {
  env KUBECTL="${MOCK_KUBECTL}" MOCK_SCENARIO="$1" NAMESPACE=obaas RELEASE_NAME=obaas \
    VALIDATION_TIMEOUT=10m SIGNOZ_VERSION=v0.142.1 COLLECTOR_VERSION=v0.144.10 \
